...
class TestUserPromptSubmit:
    def test_warns_on_pii_in_prompt(self, hook_env):
        ...
        rc, out, err = _run_hook("claude_wall.hooks.user_prompt_submit", payload, hook_env)
    ...
    def test_passthrough_clean_prompt(self, hook_env):
        ...
        rc, out, _ = _run_hook("claude_wall.hooks.user_prompt_submit", payload, hook_env)
        assert rc == 0
    ...
    def test_strict_mode_blocks(self, hook_env):
        payload = json.loads((FIX / "prompt_with_pii.json").read_text())
        env = {**hook_env, "CLAUDE_WALL_STRICT": "1"}
        rc, _, err = _run_hook("claude_wall.hooks.user_prompt_submit", payload, env)
        assert rc == 2
        assert err
